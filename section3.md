...
- 예 0.7.x Version의 Solidity(Smart Contract) 파일은 0.8.x Version에서 Compile 되지 않을수 있습니다.
...
- CA는 Contract의 주소입니다. 이 주소는 Contract를 배포하면 할당되며 이 주소를 이용하여 우리는 Contract와 Interaction할 수 있습니다.
...
- eth_call은 채굴과정이 필요하지 않아 바로 결과값을 받을수 있습니다.
...
- Contract 멤버변수
    ...
    - bool, string, uint256, array, mapping
        ...
        - private 변수는 abi에 해당변수를 참조할 수 있는 interface가 존재 하지 않으므로 컨트랙트 외부(EOA 혹은 CA)에서 참조할 수 없습니다.
...
- Contract 멤버함수
    ...
    - payable, require
        ...
        - require문을 넣어 필요한 조건을 체크하고 충족하지 않을시 transaction을 [revert](#revert)시킬수 있습니다.

- event

    - EOA가 Contract의 non view 함수를 호출할때는 returns 값을 받을수 없습니다.
...
<div id="evm_compatible_network"></div>
...
- Transaction : 여러개의 작업을 하나로 묶은 단위입니다. 트랜잭션이라고 말할수 있는 작업 묶음은 반드시 모두가 성공하거나 모두가 실패하게 됩니다. 예를 들어 은행에서 예금을 이체할때 내 잔고는 원하는 만큼 마이너스가 되야 하고 상대방 잔고는 그 만큼 플러스가 되야하는데 둘중에 한가지의 작업만 완료되면 큰 사고로 이어지게 되는데 이때 트랜잭션을 사용하여 2가지 작업 묶음 모두 성공하거나 한가지가 실패했을때 모두 실패하게되어 무결성을 보장해줍니다.
...
- revert : 트랜잭션을 실행될때 실행하는 모든 코드는 모두 실패로 만들수 있습니다. 이미 여러 코드를 실행하였다고 하여도 코드 마지막에 revert를 시키면 모든 코드가 실행되지 않은 상태로 돌릴수 있습니다. 이는 블럭체인 원리와 연관이 있으며 revert가 되었다고 해도 일부 가스비는 돌려 받지 못합니다.
